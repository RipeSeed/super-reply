...
def for_change_tone(messages: list, reply_tone: str, reply_from: str, word_count: str):
    message = "Read the following messages\n"
    message += "\n".join([msg['message'] for msg in messages])
    message += f"\nAnd suggest {reply_from if reply_from!=None else ''}a similar message in {reply_tone} tone"
    message += f"{f'of about {word_count} words' if word_count!=None else ''} "
